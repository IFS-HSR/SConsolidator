...
public final class SConsHelper {
    ...
    public static String findFileAbovePath(File startDir, String fileName) {
        if (!startDir.isDirectory()) throw new IllegalArgumentException("Not a directory given");

        List<File> filesAndDirs = list(startDir.listFiles());
        filesAndDirs.add(startDir);

        for (File file : filesAndDirs) {
            if (file.isFile() && file.getName().equals(fileName)) return file.getParentFile().getAbsolutePath();
        }

        if (startDir.getParentFile() != null) return findFileAbovePath(startDir.getParentFile(), fileName);
    ...
    public static boolean isSConsFile(IFile file) {
        ...
        return fileName.equals(SCONSCRIPT) || fileName.equals(SCONFIG_PY) || fileName.equals(SCONFIG_PYC) || fileName.equals(getSConstructFileName());
    }
