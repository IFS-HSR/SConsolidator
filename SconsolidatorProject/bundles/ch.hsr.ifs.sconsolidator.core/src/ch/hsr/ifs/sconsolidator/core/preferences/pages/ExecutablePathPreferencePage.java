...
public class ExecutablePathPreferencePage extends FieldEditorPreferencePage implements
  ...
  private SConsVersion determineSConsVersion(String path) throws EmptySConsPathException,
      IOException, InterruptedException {
    SConsVersionCommand versionCommand =
        new SConsVersionCommand(path, new BuildConsole(
  ...
  private void addOpenConsoleOption(Composite parent) {
    BooleanFieldEditor openConsoleWhenBuildingCheck =
        new BooleanFieldEditor(PreferenceConstants.OPEN_CONSOLE_WHEN_BUILDING,
  ...
  private void addClearConsoleOption(Composite parent) {
    BooleanFieldEditor clearConsoleBeforeBuildCheck =
        new BooleanFieldEditor(PreferenceConstants.CLEAR_CONSOLE_BEFORE_BUILD,
  ...
  private void setCurrentVersion() {
    versionLabel.setText(NLS.bind(SConsI18N.ExecutablePathPreferencePage_SConsVersionLabel,
        currentVersion));
  ...
  private int showConfirmationDialog() {
    String[] buttonLabels =
        {SConsI18N.ExecutablePathPreferencePage_SaveButton,
            SConsI18N.ExecutablePathPreferencePage_DiscardButton, IDialogConstants.CANCEL_LABEL};
    MessageDialog messageDialog =
        new MessageDialog(getShell(), SConsI18N.ExecutablePathPreferencePage_SaveFirstTitle, null,
            SConsI18N.ExecutablePathPreferencePage_SaveFirstMessage, MessageDialog.QUESTION,
            buttonLabels, 0);
  ...
  private class SConsExecutableEditor extends FileFieldEditor {
    SConsExecutableEditor(Composite parent) {
      super(PreferenceConstants.EXECUTABLE_PATH,
          SConsI18N.ExecutablePathPreferencePage_PathSConsExecField, false,
    ...
    private void showIncompatibleSConsError() {
      showErrorMessage(NLS.bind(SConsI18N.ExecutablePathPreferencePage_WrongSConsVersionFound,
          SConsVersion.MIN_VERSION, currentVersion));
    }
