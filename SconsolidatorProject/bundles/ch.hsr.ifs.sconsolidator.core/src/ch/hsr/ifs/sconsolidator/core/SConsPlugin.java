...
public class SConsPlugin extends AbstractUIPlugin {
    ...
    public static void showExceptionInDisplayThread(final String title, final String message, final Throwable throwable) {
        UIUtil.runInDisplayThread(new Runnable() {
    ...
    private static void showException(String title, String message, Throwable throwable) {
        IStatus status = new Status(IStatus.ERROR, SConsPlugin.getPluginId(), IStatus.OK, (throwable.getMessage() == null) ? throwable.getClass()
                .getName() : throwable.getMessage(), throwable);
        SConsPlugin.getDefault().getLog().log(status);
        ErrorDialog.openError(UIUtil.getWindowShell(), title, message, status);
    }

    private static void showException(Exception e) {
        showException(SConsI18N.UIUtils_ExceptionTitleExceptionCaughtTitle, SConsI18N.UIUtils_ExceptionTitleExceptionCaughtMessage, e);
    }
}
