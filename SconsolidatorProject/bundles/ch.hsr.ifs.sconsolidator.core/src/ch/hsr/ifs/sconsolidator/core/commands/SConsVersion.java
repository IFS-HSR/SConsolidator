...
public class SConsVersion {
  ...
  private static final Pattern VERSION_RE = Pattern.compile(ENGINE_PREFIX
      + " v(\\d)*.(\\d)*.(\\d)*(.*)");
  ...
  private SConsVersion(int majorVersion, int minorVersion, int revision) {
    if (majorVersion < 0 || minorVersion < 0 || revision < 0)
      throw new IllegalArgumentException("Version number parts should not be negative!");
  ...
  public SConsVersion(String version) {
    ...
    if (!versionMatcher.find())
      throw new IllegalArgumentException("Version string has wrong format");
