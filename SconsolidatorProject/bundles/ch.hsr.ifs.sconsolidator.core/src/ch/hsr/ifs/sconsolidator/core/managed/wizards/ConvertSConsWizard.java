...
public class ConvertSConsWizard extends Wizard implements INewWizard {
    ...
    public boolean performFinish() {
        if (convertPage == null && !isProjectSelected()) return false;

        try {
            executeConversion(getConvertableProjects());
        } catch (InvocationTargetException e) {
            SConsPlugin.log(e);
            MessageDialog.openError(getShell(), SConsI18N.ConvertSConsWizard_ErrorTitle, e.getTargetException().getMessage());
            return false;
    ...
    private void executeConversion(final IProject[] selectedProjects) throws InvocationTargetException, InterruptedException {
    ...
    private void performOperation(IProject[] selectedProjects, IProgressMonitor pm) throws CoreException {
