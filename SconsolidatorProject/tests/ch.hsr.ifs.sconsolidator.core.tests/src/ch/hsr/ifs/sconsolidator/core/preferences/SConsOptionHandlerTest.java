...
public class SConsOptionHandlerTest {
  ...
  public void testCmdLineOptionsWorkspaceDefault() {
    ...
    assertEquals(String.format("--jobs=%s", SConsHelper.getNumOfPreferredJobs()),
        commandLineOptions.iterator().next());
  }
  ...
  public void testSConsOptionsWorkspaceDefault() {
    ...
    assertEquals(String.format("{'num_jobs':'%s'}", SConsHelper.getNumOfPreferredJobs()),
        sconsOptions);
  }
  ...
  public void testCmdLineOptionsProjectSpecific() throws IOException {
    ...
    Collection<String> commandLineOptions = handler.getCommandLineOptions();
    String expected =
        "--keep-going --ignore-errors --random --silent -Q --jobs=8 --implicit-cache "
  ...
  public void testSConsOptionsSpecialized() {
    ...
    String sconsOptions = handler.getSConsFileOptions();
    String expected =
        "{'keep_going':'', 'ignore_errors':'', 'random':'', 'silent':'', 'num_jobs':'8', "
  ...
  private static void initNumberOfJobs() {
    final IPreferenceStore preferences =
        SConsPlugin.getActivePreferences(testProject.getProject(),
            PreferenceConstants.BUILD_SETTINGS_PAGE_ID);
    preferences.putValue(PreferenceConstants.NUMBER_OF_JOBS,
        "" + SConsHelper.getNumOfPreferredJobs());
  }
}
