...
public class PythonUtilTest {
  ...
  public void testToPythonList() {
    ...
    assertEquals("['eins', 'zwei', 'drei']",
        PythonUtil.toPythonList(Arrays.asList(new String[] {"eins", "zwei", "drei"})));
  }
  ...
  public void testToPythonDict() {
    ...
    assertEquals("{'color':'blue'}",
        PythonUtil.toPythonDict(Collections.singletonMap("color", "blue")));
    Map<String, String> properties = new TreeMap<String, String>();
    properties.put("color", "red");
    properties.put("name", "schiri");
    properties.put("price", "1 million");
    assertEquals("{'color':'red', 'name':'schiri', 'price':'1 million'}",
        PythonUtil.toPythonDict(properties));
  }
