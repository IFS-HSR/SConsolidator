...
public class PlatformSpecificsTest {
    ...
    public void gccErrorPatternMatchWithLinuxPath() {
        Matcher m = PlatformSpecifics.CPP_RE.matcher(
                "src/shapes/Circle.cpp:6:28: error: ISO C++ forbids declaration of 'Circleasdf' with no type [-fpermissive]");
        assertTrue(m.find());
    ...
    public void bug24HyperlinkingIssueWithGreedyRegexQuatifier() {
        Matcher m = PlatformSpecifics.CPP_RE.matcher(
                "ioengine\\DiskManager\\dmDisk.cpp(50) : fatal error C1083: Cannot open include file: 'config/access/caAttrBool.h2");
        assertTrue(m.find());
