...
public class SConsFileWriter {
    ...
    public void writeSConfig() throws CoreException {
        try {
            ...
            SConsPlugin.log(e);
            IStatus status = new Status(IStatus.ERROR, SConsPlugin.PLUGIN_ID, 0, "Error getting project details.", e);
    ...
    private void addLinkerFlags(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("LINKER_FLAGS = %s%s", toPythonStringLiteral(settings.getLinkerFlags()), NEW_LINE));
    }
    ...
    private void addPostBuildDesc(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("POST_BUILD_DESC = %s%s", toPythonStringLiteral(settings.getPostannouncebuildStep()), NEW_LINE));
    }

    private void addPostBuildCommand(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("POST_BUILD_COMMAND = %s%s", toPythonStringLiteral(settings.getPostbuildStep()), NEW_LINE));
    }

    private void addPreBuildDesc(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("PRE_BUILD_DESC = %s%s", toPythonStringLiteral(settings.getPreannouncebuildStep()), NEW_LINE));
    }

    private void addPreBuildCommand(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("PRE_BUILD_COMMAND = %s%s", toPythonStringLiteral(settings.getPrebuildStep()), NEW_LINE));
    }

    private void addSourcePaths(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("SOURCE_PATHS = %s%s", toPythonDict(settings.getSourceDirsAndExclusionPatterns()), NEW_LINE));
    }
    ...
    private void addCompilerName(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("COMPILER_NAME = %s%s", toPythonStringLiteral(settings.getCompilerName()), NEW_LINE));
    }

    private void addCxxFlags(ProjectSettingsReader settings, StringBuilder sb) throws BuildException {
        sb.append(format("CXX_FLAGS = %s%s", toPythonStringLiteral(settings.getCompilerCxxFlags()), NEW_LINE));
    }

    private void addCFlags(ProjectSettingsReader settings, StringBuilder sb) throws BuildException {
        sb.append(format("C_FLAGS = %s%s", toPythonStringLiteral(settings.getCompilerCFlags()), NEW_LINE));
    }

    private void addToolChainName(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("TOOLCHAIN_NAME = %s%s", toPythonStringLiteral(settings.getToolchainName().toLowerCase()), NEW_LINE));
    }

    private void addBuildConfiguration(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("BUILD_CONFIGURATION = %s%s", toPythonStringLiteral(settings.getDefaultConfigurationName()), NEW_LINE));
    }

    private void addProjectType(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("PROJECT_TYPE = %s%s", toPythonStringLiteral(settings.getProjectType()), NEW_LINE));
    }

    private void addBuildArtifactName(ProjectSettingsReader settings, StringBuilder sb) {
        sb.append(format("BUILD_ARTIFACT_NAME = %s%s", toPythonStringLiteral(settings.getArtifactName()), NEW_LINE));
    }

    private void addCompIncludesOptions(StringBuilder sb) {
        IPreferenceStore p = SConsPlugin.getActivePreferences(project, PreferenceConstants.PERF_VS_ACCURACY_PAGE_ID);
        sb.append(format("COMP_INCLUDES_INTO_CCFLAGS = %s%s", toPythonBoolean(p.getBoolean(PreferenceConstants.SYSTEM_HEADER_CCFLAGS_TRICK)),
                NEW_LINE));
    }

    private void addDeciderOptions(StringBuilder sb) {
        IPreferenceStore p = SConsPlugin.getActivePreferences(project, PreferenceConstants.PERF_VS_ACCURACY_PAGE_ID);
        sb.append(format("DECIDER = %s%s", toPythonStringLiteral(p.getString(PreferenceConstants.DECIDERS)), NEW_LINE));
    }

    private void addSConsOptions(StringBuilder sb) {
        sb.append(format("SCONS_OPTIONS = %s%s", new SConsOptionHandler(project).getSConsFileOptions(), NEW_LINE));
    }
