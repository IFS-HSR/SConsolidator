...
abstract class SConsCommand {
    ...
    public SConsCommand(String binaryPath, IProject project, SConsConsole console, String[] defaultArguments) throws EmptySConsPathException {
    ...
    protected void performWait(SConsProcessResultHandler handler, IProgressMonitor pm) throws InterruptedException {
    ...
    private String getProjectEnvOptions() {
        IPreferenceStore buildSettings = SConsPlugin.getActivePreferences(project, BUILD_SETTINGS_PAGE_ID);
    ...
    private String getConsoleColorInfo() {
        return NLS.bind(SConsI18N.AbstractSConsCommand_ConsoleColorInfo, SConsI18N.AbstractSConsCommand_ConsoleColorInfoLinkText);
    }
    ...
    private void raiseProblemIfNecessary(int exitValue, Exception e) {
        ...
        if (exitValue != 0 && !haveCommandsBeenExecuted()) throw new RuntimeException(AbstractSConsCommand_ProblemsOccuredCallingSCons + ": " +
                                                                                      PlatformSpecifics.NEW_LINE + getError(), e);
    }

    private boolean haveCommandsBeenExecuted() {
        for (String arg : defaultArguments)
            if (arg.equals(SCONS_COMMANDS_NOT_EXECUTED)) return false;
        return true;
    }
    ...
    protected List<String> getAdditionalSConsOptions(IProject project) {
        ...
        return StringUtil.split(PlatformSpecifics.expandEnvVariables(prefs.getString(ADDITIONAL_COMMANDLINE_OPTIONS)));
    }
