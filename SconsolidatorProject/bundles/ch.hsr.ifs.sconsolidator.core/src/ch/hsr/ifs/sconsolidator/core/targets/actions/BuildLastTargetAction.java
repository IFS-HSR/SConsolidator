...
public class BuildLastTargetAction extends ActionDelegate implements IObjectActionDelegate, IWorkbenchWindowActionDelegate {
    ...
    public void run(IAction action) {
        ...
        try {
            String lastTargetName = (String) container.getSessionProperty(new QualifiedName(SConsPlugin.getPluginId(), "lastTarget"));
    ...
    private void showTargetView() throws PartInitException {
        PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView(SConsTargetView.VIEW_ID);
    }
    ...
    private boolean isInteractive(IContainer container) throws CoreException {
        Object val = container.getSessionProperty(new QualifiedName(SConsPlugin.getPluginId(), "lastTargetInteractive"));
    ...
    private void handleEditorSelection() {
        container = null;
        IWorkbenchPart part = workbenchPart != null ? workbenchPart : windowPart.getActivePage().getActivePart();
