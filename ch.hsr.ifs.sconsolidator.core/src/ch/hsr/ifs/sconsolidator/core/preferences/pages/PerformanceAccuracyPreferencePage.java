...
public class PerformanceAccuracyPreferencePage extends FieldEditorOverlayPage implements
  ...
  public boolean performOk() {
    ...
    if (result) {
      ...
      getPreferenceStore().setValue(PreferenceConstants.IMPLICIT_DEPS_UNCHANGED,
          implicitDepsUnchanged.getSelection());
      getPreferenceStore().setValue(PreferenceConstants.SYSTEM_HEADER_CCFLAGS_TRICK,
          includesInCCFlags.getSelection());
  ...
  private void setPerfAccuracySettings(Map<String, String> settings) {
    ...
    implicitDepsUnchanged.setSelection(Boolean.valueOf(settings
        .get(PreferenceConstants.IMPLICIT_DEPS_UNCHANGED)));
    includesInCCFlags.setSelection(Boolean.valueOf(settings
        .get(PreferenceConstants.SYSTEM_HEADER_CCFLAGS_TRICK)));
  ...
  private String getCurrentDecider() {
    if (md5DeciderButton.getSelection())
      ...
      return SConsDecider.TIMESTAMP_NEWER.toString();
  }

  private void initFields() {
    useCache.setSelection(getPreferenceStore().getBoolean(PreferenceConstants.USE_CACHE));
    implicitDepsUnchanged.setSelection(getPreferenceStore().getBoolean(
        PreferenceConstants.IMPLICIT_DEPS_UNCHANGED));
    includesInCCFlags.setSelection(getPreferenceStore().getBoolean(
        PreferenceConstants.SYSTEM_HEADER_CCFLAGS_TRICK));
  ...
  private void createPerfRelatedBuildOptions() {
    accuracyVsSpeed =
        new RadioGroupFieldEditor(PreferenceConstants.PERF_ACCURACY_PROFILE,
            SConsI18N.PerformanceAccuracyPreferencePage_Profiles, 3, ACCURACY_VS_SPEED, parent);
    addField(accuracyVsSpeed, parent);
    fieldEditors = new LinkedList<FieldEditor>();
    expertMode =
        new BooleanFieldEditor(PreferenceConstants.EXPERT_MODE,
            SConsI18N.SettingsPreferencePage_ExpertMode, parent) {
          @Override
    ...
    useCache =
        createNewCheckbox(parent, SConsI18N.SettingsPreferencePage_ImplicitCacheOptionName,
            SConsI18N.PerformanceAccuracyPreferencePage_ImplicitCacheOptionTooltip);
    implicitDepsUnchanged =
        createNewCheckbox(parent, SConsI18N.SettingsPreferencePage_ImplicitDependenciesUnchanged,
            SConsI18N.PerformanceAccuracyPreferencePage_ImplicitDependenciesUnchangedTooltip);
    includesInCCFlags =
        createNewCheckbox(parent,
            SConsI18N.PerformanceAccuracyPreferencePage_SystemIncludesCCFlagsTrickOptionName,
