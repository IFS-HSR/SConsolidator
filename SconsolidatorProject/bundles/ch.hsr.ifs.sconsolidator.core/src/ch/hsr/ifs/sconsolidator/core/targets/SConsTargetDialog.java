...
public class SConsTargetDialog extends Dialog {
    ...
    private String getTitle() {
        return target == null ? SConsI18N.SConsTargetDialog_CreateTargetTitle : SConsI18N.SConsTargetDialog_ModifyTargetTitle;
    }
    ...
    private void createTargetControl(Composite parent) {
        ...
        sameAsNameCheckBox.setSelection(targetDescription.equals(targetName) || (targetDescription.length() == 0 && targetName == null));
        sameAsNameSelected();
    }
    ...
    private void createAdditionalBuildArgsControl(Composite parent) {
        Group group = createGroup(parent, SConsI18N.SConsTargetDialog_AdditionalCommandLineGroupLabel, 1);
        GridLayout layout = new GridLayout(2, false);
        group.setLayout(layout);
        GridData gd = new GridData(GridData.FILL_HORIZONTAL);
        gd.widthHint = convertWidthInCharsToPixels(50);
        ...
        ((GridData) (additionalCommandLineArgsText.getLayoutData())).horizontalAlignment = GridData.FILL;
    ...
    private void updateButtons() {
        if (getButton(IDialogConstants.OK_ID) != null) {
            getButton(IDialogConstants.OK_ID).setEnabled(targetHasChanged() && !statusLine.hasErrorMessage());
        }
    }

    private boolean targetHasChanged() {
        if (initializing || target == null || !SConsPlugin.getDefault().getSConsTargetManager().targetExists(target)) return true;

        if (!targetName.equals(getTargetName()) || !targetDescription.equals(getTargetDescription()) || !additonalCommandLineArgs.equals(
                getAdditionalCommandLineArgs())) return true;
        return false;
    }
    ...
    protected void okPressed() {
        try {
            ...
            if (target == null) {
                target = targetManager.createTarget(container.getProject(), name, targetBuildID, targetDesc, addCmdLineArgs);
            } else {
