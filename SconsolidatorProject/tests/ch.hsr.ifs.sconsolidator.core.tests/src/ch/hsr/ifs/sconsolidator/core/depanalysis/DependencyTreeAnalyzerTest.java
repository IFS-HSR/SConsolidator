...
public class DependencyTreeAnalyzerTest {
    ...
    public void testExampleTree() {
        ...
        assertEquals("coast/compress/.build/Linux_glibc_2.9-i686-32/config_compress.os", children.get(0).toString());
        ...
        assertEquals("coast/foundation/base/InitFinisManager.h", children.get(1).getChildren().get(0).toString());
        ...
        assertEquals("coast/foundation/base/config_foundation.h", children.get(3).getChildren().get(0).toString());
    ...
    public void testTargetsForSourceFile() {
        DependencyTreeAnalyzer treeAnalyzer = new DependencyTreeAnalyzer(TREE_EXAMPLE);
        Collection<String> targetsForSource = treeAnalyzer.collectTargets("coast/compress/config_compress.cpp");
        assertEquals(1, targetsForSource.size());
        assertEquals("coast/compress/.build/Linux_glibc_2.9-i686-32/config_compress.os", targetsForSource.iterator().next());
    }

    @Test
    public void testTargetsForHeaderFile() {
        DependencyTreeAnalyzer treeAnalyzer = new DependencyTreeAnalyzer(TREE_EXAMPLE);
        Collection<String> targetsForHeader = treeAnalyzer.collectTargets("coast/foundation/base/config_foundation.h");
        assertEquals(2, targetsForHeader.size());
        assertTrue(targetsForHeader.contains("coast/compress/.build/Linux_glibc_2.9-i686-32/config_compress.os"));
        assertTrue(targetsForHeader.contains("coast/compress/.build/Linux_glibc_2.9-i686-32/config_compress_ext.os"));
    }
}
