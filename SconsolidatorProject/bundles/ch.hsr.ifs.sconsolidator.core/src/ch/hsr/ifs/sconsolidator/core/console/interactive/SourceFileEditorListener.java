...
class SourceFileEditorListener implements IPartListener {
    ...
    public SourceFileEditorListener(InteractiveConsole console, IProject project, CurrentTargetAction cleanAction,
                                    BuildCurrentTargetAction buildAction) {
    ...
    private void refreshAction(CurrentTargetAction action, TargetCommand.CommandType commandType, IFile file) {
        if (!console.isProcessStarted()) return;
    ...
    private boolean isProjectFileOpenInActiveEditor(IWorkbenchPart part) {
        ...
        if (!(activeEditor instanceof CEditor)) return false;
    ...
    private IFile getProjectFileInEditor(CEditor editor) {
        ...
        if (file.getProject().equals(project)) return file;
        return null;
    }
